    ...
    IClock clock)
{
    ...
    public async Task<Result> ScheduleAsync(Guid id, ScheduleWorkOrderRequest request, CancellationToken ct = default)
    {
        ...
        return await MutateAsync(id, order => order.Schedule(new TechnicianId(request.TechnicianId), window.Value, clock), ct);
    }
    ...
    /// <summary>
    /// Records that Scheduling confirmed the booking, closing the saga.
    /// </summary>
    /// <remarks>
    /// Goes through <c>MutateAsync</c> like every other transition, so it gets the same save and
    /// the same event dispatch. Writing the persistence by hand in the handler would have been
    /// two lines shorter and would have quietly opted out of both.
    /// </remarks>
    public Task<Result> ConfirmReservationAsync(Guid id, CancellationToken ct = default) =>
        MutateAsync(id, order => order.ConfirmReservation(clock), ct);

    /// <remarks>
    /// The read path, and deliberately the untracked one. Every caller of this is a query that
    /// serialises the result and discards it; none of them mutate. <c>MutateAsync</c> below still
    /// uses the tracked <c>GetAsync</c>, because it is about to change what it loads.
    /// </remarks>
    public async Task<WorkOrder?> GetAsync(Guid id, CancellationToken ct = default) =>
        await repository.GetForReadAsync(new WorkOrderId(id), ct);
