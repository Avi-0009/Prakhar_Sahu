...
public sealed class EfWorkOrderRepository(WorkManagementDbContext db) : IWorkOrderRepository
{
    ...
    /// <remarks>
    /// <c>AsNoTracking</c> is the whole difference. EF still runs the same SQL and materialises
    /// the same objects; what it skips is building and retaining a change-tracking entry for the
    /// aggregate root and every owned labour row.
    /// </remarks>
    public async Task<WorkOrder?> GetForReadAsync(
        WorkOrderId id, CancellationToken cancellationToken = default) =>
        await db.WorkOrders
            .AsNoTracking()
