...
using QuotesApi.Identity.Infrastructure;
...
{
    "api" => QuotesApi.SharedKernel.Observability.Telemetry.ApiRoleName,
    "worker" => QuotesApi.SharedKernel.Observability.Telemetry.WorkerRoleName,
    _ => "quotes-all-in-one"
};
...
// COMPOSITION.
//
// This is the whole of it. The host names five modules and knows nothing about what is inside
// any of them — not a repository, not a handler, not a DbSet.
//
// Before the module split this section named concrete types directly: QuoteRepository,
// IsOwnerHandler, CachedQuoteReader, QuoteReportHandler, the JWT scheme, the authorization
// policies. Adding a service to a feature meant editing the composition root, so the
// composition root gradually became the one file that knew everything, and the only file that
// had to be understood before any change.
//
// Each AddXModule below is the module's own decision about what it needs. The host's job is
// ordering and configuration, and that is all it can express.
// =============================================================================================

// The shared database. See PersistenceExtensions for why one context is a recorded compromise
// rather than an oversight, and where the architecture tests bound it.
builder.Services.AddPersistence(builder.Configuration, builder.Environment);

// The clock. In the shared kernel because every module needs "what time is it" and none of
// them should answer it with DateTime.UtcNow scattered through their code.
builder.Services.AddSingleton<IClock, SystemClock>();

// Who the caller is: JWT and optional Entra schemes, the token options, and the auth endpoints.
...
// The core domain. Quotes, ownership, the Day 21 cache over the hot read, and this module's
// own background job handler.
...
// startup if the pipeline is misconfigured. The module also sets a job-aware shutdown timeout —
// the default 5s is shorter than the grace period a running job is given, so without it the
// host kills the process mid-job and the grace period accomplishes nothing.
builder.Services.AddJobsModule(builder.Configuration);

// Days 19 and 20: the Service Bus transport and the transactional outbox in front of it. Quote
// creation writes the event as a row in the same transaction as the quote, and the relay
// publishes it afterwards — so a crash between the commit and the publish loses nothing.
// Registers a no-op publisher when no broker is configured, so the API still runs on a laptop.
...
builder.Services.AddResilienceModule(builder.Configuration);
...
var otel = builder.Services.AddOpenTelemetry() // registers openTelemetry
    ...
    .WithTracing(t =>
    {
        ...
        foreach (var source in QuotesApi.SharedKernel.Observability.Telemetry.SourceNames)
        {
            t.AddSource(source);
        }
    ...
    .WithMetrics(m =>
    {
        m.AddMeter(QuotesApi.SharedKernel.Observability.Telemetry.MeterName)
            .AddAspNetCoreInstrumentation()
            .AddHttpClientInstrumentation()
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (isApiRole && !db.Quotes.Any())
    {
        ...
        var clock = scope.ServiceProvider.GetRequiredService<QuotesApi.SharedKernel.IClock>();
        var seeded = 0;
...
if (isApiRole)
{
    ...
    // Each module maps its own surface. The host does not know how many endpoints that is, or
    // what they are called — only that the module has one.
    v1.MapIdentityModule();
    v1.MapQuotesModule();
    v1.MapJobsModule();
    v1.MapMessagingModule();
    ...
    if (app.Environment.IsDevelopment())
    {
        // Named separately from the modules' production surface so neither can be mapped by
        // accident. A module that wanted to expose a debug lever has to say so twice.
        v1.MapQuotesDevEndpoints();
        app.MapResilienceDevEndpoints();
        app.Logger.LogWarning(
