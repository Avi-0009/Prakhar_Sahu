...
public class LayerDependencyTests
{
    ...
    /// <summary>
    /// Provider-specific migrations, which are a real project category and not a loophole.
    /// </summary>
    /// <remarks>
    /// <para>
    /// EF Core migrations are generated against one provider and encode that provider's column
    /// types. The set inside <see cref="SharedPersistence"/> was generated against SQLite and
    /// declares TEXT and INTEGER columns; replaying it on SQL Server produces a schema that is
    /// created successfully and is wrong. EF discovers migrations by scanning an assembly and
    /// cannot tell two providers' sets apart inside one, so the second set needs its own.
    /// </para>
    /// <para>
    /// It is allowed to reference the shared database and nothing else, which is stricter than
    /// the Infrastructure projects are held to. Generated code only.
    /// </para>
    /// </remarks>
    private const string MigrationsSuffix = "QuotesApi.Persistence.";
    ...
    public void Every_project_is_classifiable()
    {
        ...
        {
            var ok = IsMigrationsProject(p.Name)
                || (SolutionGraph.IsModuleProject(p.Name)
    ...
    public void The_shared_database_is_the_only_compromise_and_it_is_bounded()
    {
        ...
        var wrongConsumers = SolutionGraph.Projects
            ...
            .Where(p => p.Name != "QuotesApi.Host"
                        && !IsMigrationsProject(p.Name)
                        && SolutionGraph.LayerOf(p.Name) != "Infrastructure")
            .Select(p => p.Name)
            .ToArray();
    ...
    /// <summary>
    /// A migrations assembly holds generated migrations and nothing else.
    /// </summary>
    /// <remarks>
    /// The rule above exempts these projects from the "only Infrastructure may touch the database"
    /// constraint. This is the price of that exemption: they may reference the shared database and
    /// may reference nothing else at all. Without this, "it is a migrations project" would become
    /// a way to reach anything from anywhere.
    /// </remarks>
    [Fact]
    public void A_migrations_assembly_touches_the_database_and_nothing_else()
    {
        foreach (var p in SolutionGraph.Projects.Where(p => IsMigrationsProject(p.Name)))
        {
            var illegal = p.References.Where(r => r != SharedPersistence).ToArray();

            Assert.True(illegal.Length == 0,
                $"{p.Name} is a migrations assembly, so it may reference only {SharedPersistence}, "
                + "but also references: " + string.Join(", ", illegal));
        }
    }

    /// <summary>
    /// True for a provider-specific migrations assembly, e.g. QuotesApi.Persistence.SqlServer.
    /// </summary>
    private static bool IsMigrationsProject(string name) =>
        name.StartsWith(MigrationsSuffix, StringComparison.Ordinal) && name != SharedPersistence;
