...
namespace QuotesApi.Host.Security;
...
public static class ApiHardening
{
    ...
    /// <remarks>
    /// The NAME lives in <see cref="RateLimitPolicies"/> in the shared kernel, because the
    /// Identity module has to ask for this policy by name and must not reference the host.
    /// The limits themselves stay here: how many requests per minute is a deployment
    /// decision, not a contract.
    /// </remarks>
    public const string AuthPolicy = RateLimitPolicies.Auth;

    /// <summary>Rate-limit policy for everything else.</summary>
    public const string GlobalPolicy = RateLimitPolicies.Global;
    ...
    public static IApplicationBuilder UseSecurityHeaders(this IApplicationBuilder app) =>
        app.Use(async (context, next) =>
        {
            ...
            // HSTS. Day 32's ZAP baseline against the LIVE deployment flagged its absence
            // (10035 x3) — the header set above was written on Day 27 against a local HTTP
            // listener, where HSTS is meaningless and was therefore never missed.
            //
            // Container Apps terminates TLS and redirects http to https, which handles every
            // request except the one that matters: a browser that has never seen this host makes
            // its FIRST request in clear text and can be intercepted before the redirect
            // arrives. HSTS closes that one request, and only from the second visit onwards —
            // which is the entire reason `preload` exists rather than being decoration.
            //
            // Set unconditionally rather than behind app.UseHsts(), which excludes localhost by
            // default: that exclusion is a convenience for development and a silent hole in a
            // container whose hostname is not localhost.
