        ...
        services.AddDbContext<AppDbContext>((serviceProvider, options) =>
        {
            if (IsSqlServer(configured))
            {
                options.UseSqlServer(configured, sql =>
                {
                    ...
                    // Migrations are PROVIDER-SPECIFIC. The set in this assembly was generated
                    // against SQLite and declares its columns as TEXT and INTEGER; replaying it
                    // against SQL Server creates a schema successfully and creates the wrong one.
                    // EF refuses rather than doing that, with "the model has pending changes" --
                    // a message that sounds like a missing migration and is really a missing
                    // PROVIDER.
                    //
                    // So the SQL Server migrations live in their own assembly and only this branch
                    // knows about it. One model, two migration sets, selected by the same
                    // connection-string shape that selects the provider.
                    sql.MigrationsAssembly("QuotesApi.Persistence.SqlServer");
                });
            }
            else
            {
