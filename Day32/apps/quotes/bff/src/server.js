...
// -------------------------------------------------------------------------------------------------
// Response hardening, every one of these traceable to a finding from the live ZAP baseline scan.
//
// The broker sat behind a hardened nginx and in front of a hardened API and had almost none of
// this itself, which is the middle-tier blind spot: it is the only hop a browser talks to directly
// for every /api call, so its headers are the ones that apply to those responses.
// -------------------------------------------------------------------------------------------------

// ZAP 10037: "Server Leaks Information via X-Powered-By". Express advertises itself on every
// response. It tells an attacker the stack and costs one line to remove.
app.disable('x-powered-by');

app.use((_req, res, next) => {
  // ZAP 10035: HSTS was missing. Container Apps terminates TLS and redirects, but a browser that
  // has never seen this host still makes its FIRST request over http and can be intercepted
  // before the redirect. HSTS is what closes that one request, and only after the first visit --
  // which is why `preload` matters rather than being decoration.
  res.setHeader('Strict-Transport-Security', 'max-age=63072000; includeSubDomains; preload');

  // ZAP 10055: no CSP at all. This is a JSON API, so the policy can be maximally restrictive:
  // it serves nothing a browser should ever render, script, or frame. `default-src 'none'` is
  // the correct policy for an endpoint that only ever returns data.
  res.setHeader(
  ...
  // ZAP 10049: "Storable and Cacheable Content". An authenticated API response cached by an
  // intermediary is one user's data served to the next. `no-store` is the only safe answer for a
  // proxy that forwards per-user responses.
