...
public class SecurityTests(DispatchApiFactory factory)
{
    ...
    // Added on Day 32 after the live ZAP baseline found all three missing. They are listed here
    // rather than only fixed, because Day 31's rule was that every header in the hardening set is
    // asserted by a test -- a header added once and never asserted is a header that comes back off
    // in six months and nobody notices until the next scan.
