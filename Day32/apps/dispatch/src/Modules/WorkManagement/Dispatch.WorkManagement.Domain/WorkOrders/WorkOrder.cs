...
public sealed class WorkOrder : AggregateRoot<WorkOrderId>
{
    ...
    /// <summary>Longest accepted summary. Matches the nvarchar(500) column exactly.</summary>
    ...
    /// <summary>When Scheduling confirmed the booking, or null while we are still waiting.</summary>
    public DateTimeOffset? ReservationConfirmedAt { get; private set; }

    /// <summary>When to give up waiting for Scheduling to answer.</summary>
    public DateTimeOffset? ReservationDeadline { get; private set; }

    /// <summary>
    /// How long Scheduling gets to answer before the booking is abandoned.
    /// </summary>
    /// <remarks>
    /// Two minutes is long enough that a slow broker or a restarting consumer is not mistaken for
    /// a failure, and short enough that a customer is not left expecting a visit that nobody is
    /// coming to. It is a constant rather than configuration because it is a business promise,
    /// not a tuning knob -- if it needs to differ per priority, that is a decision to make out
    /// loud rather than an appsettings value somebody changes quietly.
    /// </remarks>
    public static readonly TimeSpan ReservationGrace = TimeSpan.FromMinutes(2);
        ...
        IClock clock)
    {
        ...
        // -----------------------------------------------------------------------------------
        // An upper bound, because the database already has one and the domain did not.
        //
        // The Summary column is nvarchar(500). Without this check a longer summary passed every
        // domain rule, reached SQL Server, and failed there -- surfacing as a 500 rather than a
        // 400. The caller was told the server had broken when in fact their request was too big.
        //
        // Worse, it is a cheap denial-of-service: an unbounded string is accepted, trimmed,
        // allocated and carried all the way to the database before anything objects.
        //
        // The constant is deliberately the same number as the column. Two different limits is a
        // bug waiting for the day somebody changes one of them.
        // -----------------------------------------------------------------------------------
        if (summary.Trim().Length > MaxSummaryLength)
        {
            return Result.Failure<WorkOrder>(WorkOrderErrors.SummaryTooLong(MaxSummaryLength));
        }
    ...
    /// fails.
    /// </remarks>
    public Result Schedule(TechnicianId technicianId, ScheduledWindow window, IClock clock)
    {
        ...
        // -------------------------------------------------------------------------------------
        // The saga now has a deadline.
        //
        // Scheduling answers with TechnicianReservedV1 or TechnicianReservationFailedV1. Nothing
        // handled the third case: NEITHER. A dropped event, a poisoned message or a crashed
        // consumer left the order in Scheduled for ever, having told a customer somebody was
        // coming. The SLA sweeper does not catch it, because it watches DUE DATES -- for a Low
        // priority order that could be days later.
        //
        // Recorded on the aggregate rather than in a saga table because it is a fact about this
        // work order: we promised a visit at a time and we are waiting for confirmation. A
        // separate store would be a second place for the truth to live.
        // -------------------------------------------------------------------------------------
        ReservationConfirmedAt = null;
        ReservationDeadline = clock.UtcNow + ReservationGrace;
    ...
    /// <summary>
    /// Scheduling confirmed the booking. The saga is complete.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Before this existed, <c>TechnicianReservedV1</c> was published and nobody subscribed —
    /// the success reply went nowhere. The order looked identical whether Scheduling had
    /// confirmed, refused, or never answered at all, so "is this booking real?" was unanswerable
    /// from the work order alone.
    /// </para>
    /// <para>
    /// Idempotent by construction: a redelivery finds the confirmation already set and returns
    /// success without raising anything. At-least-once delivery guarantees this will happen.
    /// </para>
    /// </remarks>
    public Result ConfirmReservation(IClock clock)
    {
        ArgumentNullException.ThrowIfNull(clock);
        ...
        // Deliberately tolerant about status. The confirmation is a fact that arrived, and by the
        // time it does a dispatcher may already have started the job. Refusing it then would
        // discard true information to satisfy a state machine.
        if (Status is not (WorkOrderStatus.Scheduled or WorkOrderStatus.InProgress))
        {
            return Result.Failure(
                WorkOrderErrors.WrongStatus("confirm the reservation for", Status, WorkOrderStatus.Scheduled));
        }

        ReservationConfirmedAt = clock.UtcNow;
        return Result.Success();
    }

    /// <summary>
    /// True when Scheduling was asked for a technician and has not answered in time.
    /// </summary>
    public bool IsAwaitingReservation(DateTimeOffset asOf) =>
        Status == WorkOrderStatus.Scheduled
        && ReservationConfirmedAt is null
        && ReservationDeadline is not null
        && asOf > ReservationDeadline;
