...
public static class ApiHardening
{
    ...
    public static IApplicationBuilder UseSecurityHeaders(this IApplicationBuilder app) =>
        app.Use(async (context, next) =>
        {
            ...
            // HSTS. Day 31 wrote this header set against a local HTTP listener, where HSTS is
            // meaningless, so it was never missed. Day 32's ZAP baseline against the live
            // deployment found the same omission in the Quotes API (10035 x3), and the same
            // reasoning applies here: Container Apps redirects http to https, which covers every
            // request except a first visit that can be intercepted before the redirect lands.
            headers["Strict-Transport-Security"] = "max-age=63072000; includeSubDomains; preload";

            // Also missing, and flagged on the sibling API: the browser-feature and isolation
            // headers. This API returns JSON and never needs any of them, so the strictest
            // values cost nothing.
