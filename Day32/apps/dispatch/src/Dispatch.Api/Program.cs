...
// Day 31 security re-check. Rate limiting and the body cap; headers are applied below.
builder.Services.AddApiHardening();

// Day 32: authentication and authorization, which Day 31 deliberately left out until the roles
// existed. Absent configuration disables the scheme entirely -- see DispatchAuth for why that is
// the same decision messaging and caller-identity make elsewhere, and not a back door.
builder.Services.AddDispatchAuth(builder.Configuration);
...
// Order matters and is not arbitrary. Authentication must run before authorization, and both must
// run after the rate limiter: an unauthenticated flood should be refused by the cheap check, not
// after the expensive one has validated a signature and fetched signing keys.
if (DispatchAuth.IsConfigured(builder.Configuration))
{
